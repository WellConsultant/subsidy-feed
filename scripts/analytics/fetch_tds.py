...
<header><h1>tds.fp-1.info アナリティクス</h1><p>{start}〜{today}（過去28日）／更新 {now.strftime('%Y-%m-%d %H:%M')}／GA4 {MEASUREMENT_ID}</p></header><main>
...
<section class="grid"><div class="panel"><h2>地域</h2><table><thead><tr><th>地域</th><th>ユーザー</th></tr></thead><tbody>{regions}</tbody></table></div><div class="panel"><h2>確認対象</h2><table><tbody><tr><th>サイト</th><td>https://tds.fp-1.info/</td></tr><tr><th>集計条件</th><td>hostname = tds.fp-1.info</td></tr><tr><th>期間</th><td>過去28日</td></tr></tbody></table></div></section>
